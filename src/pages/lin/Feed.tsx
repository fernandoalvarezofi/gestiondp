...
export default function Feed() {
  ...
  return (
    <div className="mx-auto -mt-4 grid max-w-7xl gap-6 md:-mt-6 lg:grid-cols-[240px_minmax(0,1fr)_320px]">
      {/* COLUMNA IZQUIERDA — solo desktop lg+ */}
      <aside className="sticky top-4 hidden h-fit space-y-4 lg:block">
        {/* Tarjeta de perfil */}
        <div className="overflow-hidden rounded-2xl border bg-card">
          <div className="h-14 w-full overflow-hidden">
            {miPerfil?.portada_url ? (
              <img src={miPerfil.portada_url} alt="" className="h-full w-full object-cover" />
            ) : (
          ...
          <div className="px-4 pb-4">
            ...
            <Link to="/lin/perfil" className="block text-sm font-bold leading-tight hover:underline">
              {miPerfil?.nombre || "Tu perfil"}
            </Link>
            <p className="mt-0.5 line-clamp-1 text-[11px] text-muted-foreground">
              {miPerfil?.actualmente || miPerfil?.tipo || ""}
            </p>
            {miPerfil?.bio && (
            ...
            <div className="mt-3 grid grid-cols-2 gap-1 border-t pt-3">
              <div>
                <p className="text-sm font-bold tabular-nums">{miPerfil?.total_seguidores ?? 0}</p>
                <p className="text-[10px] text-muted-foreground">Conexiones</p>
              </div>
              <div>
                <p className="text-sm font-bold tabular-nums">{miPerfil?.total_publicaciones ?? 0}</p>
        ...
        <nav className="overflow-hidden rounded-2xl border bg-card py-2">
          {NAV_ITEMS.map((item) => (
            <NavLink
              key={item.to}
              to={item.to}
              ...
              className={({ isActive }) =>
                `flex items-center gap-3 border-l-2 px-4 py-2.5 text-sm transition-colors ${
                  isActive
                    ? "border-foreground bg-secondary/60 font-semibold text-foreground"
          ...
          <NavLink
            to="/lin/configuracion"
            className="flex items-center gap-3 border-l-2 border-transparent px-4 py-2.5 text-sm text-muted-foreground transition-colors hover:bg-secondary/40 hover:text-foreground"
          >
          ...
          <button
            onClick={signOut}
            className="flex w-full items-center gap-3 border-l-2 border-transparent px-4 py-2.5 text-left text-sm text-muted-foreground transition-colors hover:bg-secondary/40 hover:text-foreground"
          >
